...
@Component({
  selector: 'app-test-matrix-execution',
  standalone: true,
  imports: [CommonModule, FormsModule, ExcelPreviewComponent, ImageEditorComponent],
  templateUrl: './test-matrix-execution.component.html',
  styleUrls: ['./test-matrix-execution.component.css']
})
export class TestMatrixExecutionComponent implements OnInit {
  ...
  constructor(private storageService: StorageService, private matrixDataService: MatrixDataService) {}

  ngOnInit(): void {
    const escenariosFromService = this.matrixDataService.getEscenarios();
    if (escenariosFromService && escenariosFromService.length > 0) {
      this.escenarios = escenariosFromService;
      this.matrixDataService.clear();
    } else {
  ...
  cargarCSV(event: Event): void { /* Implementar según lógica original */ }
      ...
      if (esc.evidencias && esc.evidencias.length > 0) {
        ...
        for (const ev of esc.evidencias) {
          ...
          try {
            ...
            if (!mustSplit) {
              const containerHeight = scaledHeight + 50;
